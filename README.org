...
- =:test= - test function used for comparing items; only applicable to heading items and items and the primary key; for heading items, you will need to create the test with ~define-hash-table-test~ if it does not exist (default: ~equal~; generally should be unnecessary to change)
